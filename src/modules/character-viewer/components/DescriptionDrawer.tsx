...
import { actionSymbol, getAonSearchUrl, traditionColor } from '../helpers'
...
export type DescriptionRequest =
    ...
    | { type: 'spell'; name: string; level?: number; cached?: SpellDescription }
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <Box sx={{ p: { xs: 2.5, md: 3 }, overflowY: 'auto', flex: 1 }}>
                        ...
                        {request.type === 'spell' && state.spell && (
                            <SpellMetadata spell={state.spell} accent={accent} />
                        )}
                        ...
                        {state.loading ? (
                            ...
                            <DescriptionBody
                                parts={parsed?.parts ?? []}
                                heightened={mergeHeightened(state.spell?.heightened, parsed?.heightened)}
                                accent={accent}
                            />
                        )}
...
const SpellMetadata = ({ spell, accent }: { spell: SpellDescription; accent: string }) => (
    <Box sx={{ mb: 2.5 }}>
...
const MetaRow = ({ label, value }: { label: string; value: string }) => (
    <Box sx={{ display: 'flex', gap: 0.75, alignItems: 'baseline', minWidth: 0 }}>
        ...
        <Typography component="span" sx={{ fontSize: '0.85rem', wordBreak: 'break-word' }}>
            {value}
        </Typography>
    </Box>
)
